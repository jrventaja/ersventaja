defmodule Ersventaja.Policies.OCR.PatternMatchers.Mapfre do
  @moduledoc """
  Pattern matcher for Mapfre insurance policies.
  ...
  @name_patterns [
    ...
    # Look for all caps names with at least 4 words (common in insurance documents)
    ~r/^([A-ZÁÉÍÓÚÀÈÌÒÙÂÊÎÔÛÃÕÇ]{2,}(?:\s+[A-ZÁÉÍÓÚÀÈÌÒÙÂÊÎÔÛÃÕÇ]{2,}){3,})(?:\s|$|CPF|Vigência|Data)/m
  ]
  ...
  defp extract_dates(text) do
    ...
    start_date_pattern = ~r/(?:Início|Inicio|Data de Início|Data de Inicio|Vigência de|Vigencia de|De)[\s:]+(\d{2}\/\d{2}\/\d{4})/i
    end_date_pattern = ~r/(?:Fim|Término|Termino|Data de Término|Data de Termino|Vigência até|Vigencia até|até)[\s:]+(\d{2}\/\d{2}\/\d{4})/i
        ...
        case Regex.run(pattern, text) do
          [_, captured_name | _] ->
            ...
            is_valid_name = String.length(cleaned_name) > 5 and
                            ...
                            not String.contains?(cleaned_name, "SEGURADORA") and
                            not String.contains?(cleaned_name, "MAPFRE")
